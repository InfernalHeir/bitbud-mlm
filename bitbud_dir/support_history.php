...
try
{
       ...
       $select_btc=$conn->prepare("SELECT * FROM `support` where `user_id`='$user_id'");
    $select_btc->execute();
...
function stop()
{
  ...
  <tbody>
    ...
    <td data-label="Tracking-ID"><?php echo $dir['tracking_id'];    ?></td>
    
    <td data-label="Processing Time"><?php echo $dir['processing_time'];   ?></td>
    <td data-label="Admin Follow Through"><button style="width:90px; height:auto !important;" class='<?php $status=$dir['status'];
    if($status=="SOLVED")
    {
    ...
    <td data-label="Status"><button style="width:90px; height:auto !important;" class='
    <?php
    $status=$dir['status'];
    if($status=="PENDING")
    {
